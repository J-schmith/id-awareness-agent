...
export default function Topbar({ title, subtitle }: TopbarProps) {
  ...
  return (
    <>
      <header className="sticky top-0 z-40 h-15 bg-white/80 backdrop-blur-xl border-b border-black/5 flex items-center justify-between px-7">
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            className="flex items-center gap-1.5 h-9 px-4 rounded-lg bg-[#0071e3] text-white text-[13px] font-medium hover:bg-[#0077ED] active:bg-[#006ACC] transition-colors shadow-sm"
          >
      ...
      <RunAgentModal open={agentModalOpen} onClose={() => setAgentModalOpen(false)} />
    </>
  );
}
